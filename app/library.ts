...
export async function loadLibrary():Promise<Library>{
 const data=await accountRequest<{content:Library|null}>('/api/library');
 ...
 const places=new Map(monuments.map(place=>[place.id,place as Place]));
 for(const place of published?.places||[])places.set(place.id,place);
 const saved=data.content;
 const combined=saved?[...saved.places,...Array.from(places.values()).filter(p=>!saved.places.some(s=>s.id===p.id))]:Array.from(places.values());
 const selected=combined.slice(0,100);
 const timelines={...events,...published?.timelines,...saved?.timelines};
 return resolveMedia({places:selected,timelines:Object.fromEntries(selected.map(p=>[p.id,timelines[p.id]])),media:saved?.media||{}});
}

...
export async function loadPreferences(){const values=await accountRequest<{saved:string[];visited:string[]}>('/api/preferences');const allowed=new Set(monuments.map(p=>p.id));return {saved:values.saved.filter(id=>allowed.has(id)),visited:values.visited.filter(id=>allowed.has(id))}}
